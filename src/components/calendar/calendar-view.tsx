...
import { CalendarTask } from '@/app/(dashboard)/calendar/actions'
...
export function CalendarView({ tasks }: CalendarViewProps) {
    ...
    // Get calendar days including padding for the month view
    ...
    const upcomingTasks = useMemo(() => {
        ...
        return [...filtered].sort((a, b) => {
            ...
            if (sortBy === 'status') {
                const orderA = statusColors[a.status]?.order || 99
                const orderB = statusColors[b.status]?.order || 99
                ...
                <CardContent className="p-8">
                    ...
                    <div className="grid grid-cols-7 mb-4">
                        {weekDays.map((day) => (
                            <div
                                key={day}
                                className="text-center text-xs font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 py-4"
                            >
                                {day}
                            </div>
                        ))}
                        ...
                        {calendarDays.map((day, idx) => {
                            ...
                            return (
                                <div
                                    key={idx}
                                    className={cn(
                                        "min-h-[140px] p-3 border rounded-2xl transition-all duration-300 relative group overflow-hidden",
                                        isCurrentMonth
                                ...
                                >
                                    ...
                                    {/* Date number */}
                                    <div className={cn(
                                        "text-lg font-bold mb-3 flex items-center justify-between relative z-10",
                                        isCurrentDay
                                    ...
                                    <div className="space-y-2 relative z-10">
                                                ...
                                                <Tooltip key={task.id}>
                                                    <TooltipTrigger asChild>
                                                        <Link href={`/projects/${task.projectId}`}>
                                                            <div className={cn(
                                                                "text-[12px] px-2 py-1.5 rounded-lg truncate cursor-pointer hover:scale-[1.02] active:scale-95 transition-all flex items-center gap-2 font-bold shadow-sm border",
                                                                "bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700",
                                                                // Use border color for status indication instead of full background
                                                                ...
                                                                <span className="truncate text-slate-600 dark:text-slate-300">{task.title}</span>
                                                            </div>
                                                        </Link>
                                                    </TooltipTrigger>
                                                    <TooltipContent side="top" className="max-w-[250px] p-4 rounded-2xl shadow-2xl border-0 bg-white dark:bg-slate-900 ring-1 ring-slate-200 dark:ring-slate-700">
                                        ...
                                        {dayTasks.length > 3 && (
                                            <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 pl-1">
